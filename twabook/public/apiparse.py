...
urli = "https://tgm.parseapp.com/texto"
...
def example_function(sender): # name "example_function" and given parameters are defined in main()
	...
	try:
		while not QUIT: # loop for messages
			...
			payload = {"text":{"id": msg.sender.id,"b": msg.text,"a": msg.sender.name}}

			r = requests.post(urli,headers=headers, data=json.dumps(payload))
